...
def demo_complex_tasks():
    ...
    complex_queries = [
        ...
        {"query": "搜索音乐目录中的音乐文件,如果找到就播放第一首", "description": "条件执行"},
        {
    ...
    for i, item in enumerate(complex_queries, 1):
        ...
        if function_calls:
            for func_call in function_calls:
                ...
                # 执行
                result = task_executor.execute(func_call["name"], func_call["arguments"])

                print(f"✅ 执行结果: {result}")

                # 返回结果给大模型
                final_response = llm_client.add_function_result(
                    func_call["id"], func_call["name"], result
                )
...
def demo_conversation_context():
    ...
    # 多轮对话
    conversations = ["帮我写一篇关于人工智能的短文", "把它保存到桌面", "文件名叫做ai_article.md"]
